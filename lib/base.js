...
class Base {
    ...
    constructor(plugin, options) {
        if (
            is.not.object(plugin) ||
            is.not.function(plugin.Client) ||
            is.not.function(plugin.Server)
        )
        ...
        this._logger = pino({
            level: this._options.log || process.env.LOG_LEVEL,
        });
        this._services = {};
    }
    ...
    _fixServiceName(service) {
        if (is.function(service) && is.function(service._name))
            return service._name();

        return `${service.name.charAt(0).toLowerCase()}${service.name.slice(
            1
        )}`;
    }
