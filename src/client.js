...
class Client extends Base {
    ...
    _onMessage(m) {
        ...
        if (is.not.string(service) || is.not.string(id))
            throw new Error('invalid message');
        else if (!this._services[service])
            throw new Error(`unknown service(${service})`);
        else if (!this._services[service].m[id])
            throw new Error(`unknown message(${id}@${service})`);

        if (err) this._services[service].m[id][1](new Error(err));
        else this._services[service].m[id][0](data);
        if (this._services[service].m[id][2])
            clearTimeout(this._services[service].m[id][2]);
    ...
    start(publisher, subscriber, services) {
        if (is.not.array(services) || is.empty(services))
            throw new Error('there is no service');

        for (let name of services) {
            const options = Object.assign({}, this._options.plugin || {});
            options.prefix = this._prefix(name);
            this._services[name] = {
                s: new this._ClientPlugin(publisher, subscriber, options),
                m: {},
            };
        ...
        return new Promise((resolve, reject) => {
            try {
                if (
                    ...
                    throw new Error(`unknown service(${service})`);
                else if (is.empty(this._services[service].s._pair))
                ...
                if (is.empty(instances))
                    throw new Error(`no pair found for service(${service})`);
                const address =
                ...
                if (timeout)
                    this._services[service].m[msg.id].push(
                        setTimeout(() => {
                            clearTimeout(this._services[service].m[msg.id][2]);
                            this._services[service].m[msg.id][1](
